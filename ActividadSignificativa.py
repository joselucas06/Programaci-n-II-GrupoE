...
def guardar_en_archivo():
    """Guarda la lista de pacientes en pacientePeso.txt"""
    with open("pacientePeso.txt", "w", encoding="utf-8") as archivo:
        for paciente in paciente_data:
            archivo.write(
                ...
                f"{paciente['Grupo Sanguineo']}|{paciente['Tipo de Seguro']}|{paciente['Centro Medico']}|{paciente['Peso Paciente']}\n"
            )
...
def cargar_desde_archivo_pacientes():
    """Carga pacientes desde pacientePeso.txt"""
    try:
...
labelPeso.grid(row=3, column=0, sticky="w", pady=5, padx=5)
PesoP = tk.Entry(frame_pacientes)
PesoP.grid(row=3, column=1, sticky="w", pady=5, padx=5)
...
treeview.heading("Peso", text="Peso del Paciente (Kg)")
